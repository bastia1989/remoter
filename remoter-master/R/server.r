...
remoter_server_eval <- function(env)
{
  ...
  ### withVisible makes the ret visible on client	
  ret <- 
  withCallingHandlers(
    tryCatch({
...
remoter_exit_server <- function()
{
  ...
  ### when should_exit is TRUE, server jumps out of the loop and stop runing evalfun.
  ### when kill_interactive_server is TRUE, runs q()
